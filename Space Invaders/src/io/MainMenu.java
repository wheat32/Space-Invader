...
public class MainMenu implements ConstantValues, GraphicsListener, FrameListener, MainMenuStrs
{
	...
	private void setUpMsgs()
	{
		...
		//Main Menu
		messages.put(0, new MainMenuMsg(Color.GREEN, TITLE, fonts.agencyFBPlain_large, 0, Stats.SCREEN_HEIGHT/26*4));
		messages.put(1, new MainMenuMsg(Color.GRAY, STORY_MODE, 0, Stats.SCREEN_HEIGHT/26*14));
		messages.put(2, new MainMenuMsg(Color.YELLOW, ARCADE_MODE, 0, Stats.SCREEN_HEIGHT/26*16));
		messages.put(3, new MainMenuMsg(Color.GRAY, OPTIONS, 0, Stats.SCREEN_HEIGHT/26*18));
		messages.put(4, new MainMenuMsg(new Color(59, 187, 255), EXIT_TO_DESKTOP, 0, Stats.SCREEN_HEIGHT/26*20));//Color blue
		//Arcade Mode
		messages.put(21, new MainMenuMsg(Color.YELLOW, INFINITY_MODE, 0, Stats.SCREEN_HEIGHT/26*14));
		messages.put(22, new MainMenuMsg(Color.GRAY, ENDLESS_WAVE_MODE, 0, Stats.SCREEN_HEIGHT/26*16));
		messages.put(23, new MainMenuMsg(Color.GRAY, BOSS_RUSH, 0, Stats.SCREEN_HEIGHT/26*18));
		messages.put(24, new MainMenuMsg(new Color(59, 187, 255), RETURN_TO_MAIN_MENU, 0, Stats.SCREEN_HEIGHT/26*20));//Blue
		//Infinity Mode
		messages.put(201, new MainMenuMsg(Color.GREEN, START_GAME, 0, Stats.SCREEN_HEIGHT/26*11));
		messages.put(202, new MainMenuMsg(new Color[]{Color.YELLOW, Color.WHITE}, 
				new String[]{DIFFICULTY, DIFFICULTIES[onDifficulty]}, 0, Stats.SCREEN_HEIGHT/26*13));
		messages.put(203, new MainMenuMsg(new Color[]{((onDifficulty == 5) ? Color.YELLOW : Color.GRAY), ((onDifficulty == 5) ? Color.WHITE : Color.GRAY)},
				new String[]{ROUNDS_BETWEEN_BOSSES, ObjectCollection.getGameManagement().getRoundsBetweenBoss()+""}, 0, Stats.SCREEN_HEIGHT/26*15));
		messages.put(204, new MainMenuMsg(new Color[]{((onDifficulty == 5) ? Color.YELLOW : Color.GRAY), ((onDifficulty == 5) ? Color.WHITE : Color.GRAY)},
				new String[]{STARTING_LIVES, ObjectCollection.getGameManagement().getStartingLives()+""}, 0, Stats.SCREEN_HEIGHT/26*17));
		messages.put(205, new MainMenuMsg(new Color[]{((onDifficulty == 5) ? Color.YELLOW : Color.GRAY), ((onDifficulty == 5) ? Color.WHITE : Color.GRAY)},
				new String[]{ALIEN_PACK_SPEED_MOD, ObjectCollection.getGameManagement().getAPackSpdMod()+""}, 0, Stats.SCREEN_HEIGHT/26*19));
		messages.put(206, new MainMenuMsg(new Color[]{((onDifficulty == 5) ? Color.YELLOW : Color.GRAY), ((onDifficulty == 5) ? Color.WHITE : Color.GRAY)},
				new String[]{WEAPON_COOLDOWN_MOD, ObjectCollection.getGameManagement().getWpnCoolMod()+""}, 0, Stats.SCREEN_HEIGHT/26*21));
		messages.put(207, new MainMenuMsg(new Color(59, 187, 255), RETURN_TO_MAIN_MENU, 0, Stats.SCREEN_HEIGHT/26*23));
	}
	
	private void displayMainMenu(Graphics2D gfx, long elapsedTime)
	{
		changeMsg = null;
		
		middleScreenString(gfx, messages.get(0));
		
		gfx.setFont(fonts.arialNarrow_small);
		gfx.setColor(Color.LIGHT_GRAY);
		gfx.drawString(("Ver: " + Stats.majorRelease + "." + Stats.minorRelease + "." + Stats.releaseRevisions), 4, Stats.SCREEN_HEIGHT-6);
	...
	{	
		...
		for(int i = 0; i < msg.getMessage().length; i++)
		{
			...
			gfx.drawString(msg.getMessage()[i], 
					(int) (Stats.SCREEN_WIDTH/2-(msg.getWholeStringBoundsWidth(gfx)/2)+msg.getXOffset(gfx, i)) + msg.getX(), 
					msg.getY());
	...
	private void printSubtitle(Graphics2D gfx, long elapsedTime)
	{
		gfx.setFont(fonts.agencyFBPlain_small);
		...
		switch(((selection+9)/10)*10)
		{
			case 10:
				gfx.drawString(SUBTITLES[0], 
						(int) (Stats.SCREEN_WIDTH/2-gfx.getFont().getStringBounds(SUBTITLES[0], gfx.getFontRenderContext()).getWidth()/2),
						(int) (fonts.agencyFBPlain_large.getStringBounds(TITLE, gfx.getFontRenderContext()).getHeight()*0.5 + Stats.SCREEN_HEIGHT/26*4));
				break;
			case 30:
				gfx.drawString(SUBTITLES[1], 
						(int) (Stats.SCREEN_WIDTH/2-gfx.getFont().getStringBounds(SUBTITLES[1], gfx.getFontRenderContext()).getWidth()/2),
						(int) (fonts.agencyFBPlain_large.getStringBounds(TITLE, gfx.getFontRenderContext()).getHeight()*0.5 + Stats.SCREEN_HEIGHT/26*4));
				break;
			case 210:
				gfx.drawString(SUBTITLES[3], 
						(int) (Stats.SCREEN_WIDTH/2-gfx.getFont().getStringBounds(SUBTITLES[3], gfx.getFontRenderContext()).getWidth()/2),
						(int) (fonts.agencyFBPlain_large.getStringBounds(TITLE, gfx.getFontRenderContext()).getHeight()*0.5 + Stats.SCREEN_HEIGHT/26*4));
				break;
			default:
				//Not a fatal error
	...
	private void drawArrow(Graphics2D gfx, Color color, String string, int y)
	{
		...
		gfx.setColor(color);
		gfx.drawString(">", (int) ((Stats.SCREEN_WIDTH/2-gfx.getFont().getStringBounds(string, gfx.getFontRenderContext()).getWidth()/2) 
				- gfx.getFont().getStringBounds(">", gfx.getFontRenderContext()).getWidth()/2 - Stats.SCREEN_WIDTH/32), y);
		gfx.setColor(oldColor);
	}
	...
	{	
		...
		if(KeyInputManagement.downKeyPressed == true && System.currentTimeMillis()-timeouts[0] >= 200 && KeyInputManagement.upKeyPressed == false)
		{
			...
			ObjectCollection.getAudio().playSound(SfxNames.MENUMOVEMENT);
		}
		...
		if(KeyInputManagement.fireKeyPressed == true && System.currentTimeMillis()-timeouts[2] >= 200)
		{
			...
			{
				ObjectCollection.getAudio().playSound(SfxNames.BEEP1);
	...
	private boolean checkSelection()
	{
		switch(selection)
		{
			...
			case 207:
				...
				return true;
			default:
				ObjectCollection.getAudio().playSound(SfxNames.MENUBADSELECT);
				return false;
		}
	}
	...
	public void graphicsCall(Graphics2D gfx, int elapsedTime)
	{
		...
		printSubtitle(gfx, elapsedTime);
		displayMainMenu(gfx, elapsedTime);
	...
	public void setInMainMenu(boolean inMainMenu)
	{
		...
		if(inMainMenu == true)
		{
			...
			ObjectCollection.getAudio().changeTrack(TrackNames.BGM2_INTRO, TrackNames.BGM2_LOOP);
		}
		else
		{
			ObjectCollection.getRenderer().removeGraphicsListener(this);
			ObjectCollection.getRenderer().removeFrameListener(this);
		}
	}
}
